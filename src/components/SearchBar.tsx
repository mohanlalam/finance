...
const TYPE_ICONS: Record<string, React.ReactNode> = {
  ...
  document: <FolderOpen size={12} className="text-slate-500" />,
};
...
export default function SearchBar({ portfolios, onNavigate }: SearchBarProps) {
    ...
    <div ref={containerRef} className="relative">
      <div className="relative">
        ...
        <input
          ...
          className="w-full bg-white border border-slate-200 rounded-xl pl-9 pr-9 py-2.5 text-sm text-slate-700 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-400 transition-all"
        />
        {query && (
          <button
      ...
      {open && filtered.length > 0 && (
        <div className="absolute top-full mt-1 left-0 right-0 bg-white border border-slate-200 rounded-xl shadow-xl z-50 max-h-80 overflow-y-auto">
          {Array.from(grouped.entries()).map(([type, items]) => (
            <div key={type}>
              <div className="px-3 py-1.5 bg-slate-50 text-[10px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-1.5 sticky top-0">
                {TYPE_ICONS[type]}
                {TYPE_LABELS[type]}
              </div>
              {items.map((r, i) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 flex items-center gap-2 hover:bg-blue-50 transition-colors ${flatIdx === selectedIdx ? 'bg-blue-50' : ''}`}
                  >
                    ...
                    <span className="text-[10px] bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded-md font-medium shrink-0">
                      {r.portfolioLabel}
                    </span>
                  </button>
                );
      ...
      {open && query.trim() && filtered.length === 0 && (
        <div className="absolute top-full mt-1 left-0 right-0 bg-white border border-slate-200 rounded-xl shadow-xl z-50 p-4 text-center">
