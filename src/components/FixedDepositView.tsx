    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-gradient-to-tr from-blue-600 to-indigo-600 rounded-2xl p-5 text-white shadow-md flex items-center justify-between">
          ...
          <Landmark size={40} className="opacity-20 shrink-0" />
        </div>

        <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-xs text-slate-400 font-semibold uppercase tracking-wider">Est. Maturity Value</p>
            <p className="text-2xl font-bold text-slate-800 mt-1">{formatINR(totalMaturity)}</p>
        ...
        <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-xs text-slate-400 font-semibold uppercase tracking-wider">Weighted Interest Rate</p>
            <p className="text-2xl font-bold text-slate-800 mt-1">{avgRate.toFixed(2)}%</p>
            <p className="text-xs text-slate-400 mt-2">Across all FDs</p>
          </div>
      ...
      <div className="bg-white border border-slate-100 rounded-2xl shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
          <h3 className="text-sm font-bold text-slate-700 uppercase tracking-wider">FD Registry</h3>
          <button
        ...
        {fixedDeposits.length === 0 ? (
          ...
          <div className="divide-y divide-slate-100">
            {fixedDeposits.map((fd) => {
              ...
              return (
                <div key={fd.id} className="p-6 hover:bg-slate-50/50 transition-colors">
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex items-start gap-3">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${isMatured ? 'bg-emerald-50 text-emerald-600' : 'bg-blue-50 text-blue-600'}`}>
                        {isMatured ? <CheckCircle size={20} /> : <Landmark size={20} />}
                      </div>
                      <div>
                        <div className="flex items-center gap-2 flex-wrap">
                          <h4 className="text-sm font-bold text-slate-800">{fd.bank_name}</h4>
                          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${isMatured ? 'bg-emerald-100 text-emerald-700' : 'bg-blue-100 text-blue-700'}`}>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-6 md:text-right">
                      <div>
                        ...
                        <p className="text-sm font-bold text-slate-800">{formatINR(Number(fd.principal_amount))}</p>
                      </div>
                      <div>
                        <p className="text-xs text-slate-400">{fd.maturity_date ? 'Maturity Value' : 'Current Value'}</p>
                        <p className="text-sm font-bold text-slate-800">{formatINR(getFDEffectiveValue(fd))}</p>
                      </div>
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">Enter details to lock and calculate returns</p>
              </div>
              <button
            ...
            <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
              <div>
                <label className="block text-xs font-semibold text-slate-500 mb-1.5">Bank / Issuer Name</label>
                <input
                  type="text"
                  placeholder="e.g. HDFC Bank, SBI"
                  value={bankName}
                  onChange={(e) => setBankName(e.target.value)}
                  className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-400 transition-colors"
                />
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-slate-500 mb-1.5">Principal Amount (₹)</label>
                  <input
                    type="number"
                    placeholder="0"
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-500 mb-1.5">Interest Rate (% p.a.)</label>
                  <input
                    type="number"
                    step="0.01"
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-500 mb-1.5">Maturity Date</label>
                  <input
                    type="date"
                    value={maturityDate}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-slate-500 mb-1.5">Maturity Amount (₹)</label>
                  <input
                    type="number"
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-700 bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-400 transition-colors"
                  >
