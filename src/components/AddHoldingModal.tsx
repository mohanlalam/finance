...
export default function AddHoldingModal({ onClose, onAdd, portfolioOptions, defaultPortfolio }: AddHoldingModalProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <div>
            <h2 className="text-base font-bold text-slate-800">Add New Holding</h2>
            <p className="text-xs text-slate-400 mt-0.5">Stock will be fetched from Yahoo Finance on next refresh</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          <div>
            <label className="block text-xs font-semibold text-slate-500 mb-1.5">Portfolio</label>
            <select
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-500 mb-1.5">Stock / ETF Name</label>
            <input
              type="text"
              placeholder="e.g. Reliance Industries Limited"
              value={form.stockName}
              onChange={(e) => set('stockName', e.target.value)}
              className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-700 placeholder-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-400 transition-colors"
            />
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-semibold text-slate-500 mb-1.5">Ticker Symbol</label>
              <input
                type="text"
                placeholder="e.g. RELIANCE"
                value={form.ticker}
                onChange={(e) => set('ticker', e.target.value.toUpperCase())}
                className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-700 placeholder-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-400 transition-colors uppercase"
              />
            </div>
            <div>
              <label className="block text-xs font-semibold text-slate-500 mb-1.5">Exchange</label>
              <select
                value={form.exchange}
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-500 mb-1.5">
              ...
              <span className="font-normal text-slate-400 ml-1">(auto-filled, edit if needed)</span>
            </label>
            <input
              type="text"
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-500 mb-1.5">Avg Buy Price (₹)</label>
              <input
                type="number"
                placeholder="0.00"
                min="0"
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-500 mb-1.5">
              Amount Invested (₹)
              <span className="font-normal text-slate-400 ml-1">(auto-computed from qty × avg price)</span>
            </label>
            <input
              type="number"
              placeholder="0.00"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-semibold text-slate-500 mb-1.5">52W Low (₹) <span className="font-normal text-slate-400">optional</span></label>
              <input
                type="number"
                placeholder="0.00"
                min="0"
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-500 mb-1.5">52W High (₹) <span className="font-normal text-slate-400">optional</span></label>
              <input
                type="number"
                placeholder="0.00"
                min="0"
