...
export const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(({
  ...
  const baseClasses = 'inline-flex items-center justify-center font-semibold rounded-[10px] ios-press transition-all duration-150 outline-none focus-visible:ring-2 focus-visible:ring-blue-600 focus-visible:ring-offset-2 dark:focus-visible:ring-offset-slate-900 disabled:opacity-50 disabled:cursor-not-allowed select-none';
  ...
  const variantClasses: Record<ButtonVariant, string> = {
    ...
    secondary: 'bg-slate-100 text-slate-800 hover:bg-slate-200 active:bg-slate-300 dark:bg-slate-800 dark:text-slate-200 dark:hover:bg-slate-700 border border-slate-200/80 dark:border-slate-700/80',
  ...
  const sizeClasses: Record<ButtonSize, string> = {
    sm: 'min-h-[36px] px-3 py-1.5 text-xs gap-1.5 sm:min-h-[32px]',
    md: 'min-h-[44px] px-4 py-2 text-sm gap-2 sm:min-h-[40px]',
