...
export function validateBackupJSON(
  ...
  const parsedDataObj = parsedObj?.data && typeof parsedObj.data === 'object' ? (parsedObj.data as Record<string, unknown>) : null;
  const portfoliosList: RawPortfolio[] = Array.isArray(data)
    ...
    : Array.isArray(parsedDataObj?.portfolios)
    ? (parsedDataObj?.portfolios as RawPortfolio[])
    : [];
  ...
  existingPortfolios.forEach((p) => {
    ...
    p.fixedDeposits?.forEach((fd) =>
      existingFDNames.add(`${p.name}:${fd.bank_name.toLowerCase()}:${fd.principal_amount}`)
    );
    p.goldHoldings?.forEach((g) =>
      existingGoldItems.add(`${p.name}:${g.item_name.toLowerCase()}:${g.weight_grams}`)
    );
  ...
  if (missingLinkedDocs.length > 0) {
    warnings.push(
      `${missingLinkedDocs.length} vault document references point to asset IDs not present in this backup.`
    );
  }
...
export function validateBackupJson(jsonString: string): ValidationResult {
  ...
  const report = validateBackupJSON(jsonString);
  return {
    isValid: report.isValid,
    portfolioCount: report.portfolioCount,
    totalAssetsCount: report.counts.totalAssets,
