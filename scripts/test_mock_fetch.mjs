...
async function test() {
  ...
  const targets = await fetch('http://127.0.0.1:9446/json').then(r => r.json());
  ...
  // Inject fetch interceptor on every page load via Page.addScriptToEvaluateOnNewDocument
  await sendCDP('Page.addScriptToEvaluateOnNewDocument', {
    source: `
      // Auto unlock PIN session
  ...
  console.log('DOM Info with mock fetch:', domInfo.result.value);

  // Take test screenshot
  const result = await sendCDP('Page.captureScreenshot', { format: 'png' });
  fs.writeFileSync('test_stocks.png', Buffer.from(result.data, 'base64'));
  console.log('Saved test_stocks.png');
