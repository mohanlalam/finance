...
async function test() {
  const tempDir = path.resolve('.chrome_no_pin_profile');
  ...
  const targets = await fetch('http://127.0.0.1:9452/json').then(r => r.json());
  ...
  // Intercept before page runs:
  // 1. DO NOT set custom_app_pin_hash (so !isPinConfigured() is TRUE)
  // 2. Intercept fetch to return mock data with 200 OK
  await sendCDP('Page.addScriptToEvaluateOnNewDocument', {
    source: `
      // Ensure no custom PIN is set
      try {
        localStorage.removeItem('custom_app_pin_hash');
        localStorage.removeItem('custom_app_pin_length');
      } catch (e) {}
  ...
  fs.writeFileSync('test_unlocked_success.png', Buffer.from(result.data, 'base64'));
  console.log('Saved test_unlocked_success.png');
