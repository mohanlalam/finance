...
async function test() {
  const tempDir = path.resolve('.chrome_cdp_fetch');
  ...
  const targets = await fetch('http://127.0.0.1:9455/json').then(r => r.json());
  ...
  ws.onmessage = async (evt) => {
    ...
    // Handle CDP Network Interception via Fetch domain
    if (data.method === 'Fetch.requestPaused') {
      const { requestId, request } = data.params;
      const url = request.url;
      console.log('[CDP FETCH INTERCEPTED]', url);

      let responseBody = null;

      if (url.includes('load-portfolios')) {
      ...
      if (responseBody) {
        await sendCDP('Fetch.fulfillResponse', {
          requestId,
          responseCode: 200,
          responseHeaders: [
  ...
  await sendCDP('Fetch.enable', {
    patterns: [{ urlPattern: '*functions/v1/*' }, { urlPattern: '*load-portfolios*' }]
  });

  // Navigate to application
  console.log('Navigating to app with CDP Fetch Interception...');
  ...
  console.log('DOM Info with CDP interception:', domInfo.result.value);

  const screenshot = await sendCDP('Page.captureScreenshot', { format: 'png' });
  fs.writeFileSync('test_unlocked_cdp_perfect.png', Buffer.from(screenshot.data, 'base64'));
  console.log('Saved test_unlocked_cdp_perfect.png');
