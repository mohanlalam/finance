...
async function test() {
  const tempDir = path.resolve('.chrome_mock_profile2');
  ...
  const targets = await fetch('http://127.0.0.1:9450/json').then(r => r.json());
  ...
  // Add script to run before ANY document code
  await sendCDP('Page.addScriptToEvaluateOnNewDocument', {
    source: `
      // Always set session as verified
      try {
        sessionStorage.setItem('finance_pin_verified', 'true');
        sessionStorage.setItem('finance_hashed_pin', 'mock_hash');
        localStorage.setItem('custom_app_pin_hash', 'mock_hash');
        localStorage.setItem('finance_last_family_tab', 'all');
        localStorage.setItem('finance_last_asset_tab', 'stocks');
      } catch (e) {}
  ...
  const domInfo = await sendCDP('Runtime.evaluate', {
    expression: `({
      ...
      hasHoldings: document.body.innerText.includes('Reliance') || document.body.innerText.includes('RELIANCE'),
      isPinScreen: document.body.innerText.includes('Enter Passcode')
    })`,
    returnByValue: true
  });

  console.log('DOM check with proper setup:', domInfo.result.value);

  const result = await sendCDP('Page.captureScreenshot', { format: 'png' });
  fs.writeFileSync('test_unlocked_stocks_final.png', Buffer.from(result.data, 'base64'));
  console.log('Saved test_unlocked_stocks_final.png');
