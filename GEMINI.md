...
* **Workers & Storage**:
  ...
  * **[supabaseStorage.ts](src/utils/supabaseStorage.ts)** & **[SupabaseDocumentStorageRepository.ts](src/infrastructure/supabase/repositories/SupabaseDocumentStorageRepository.ts)**: Supabase Document Storage with client-side path traversal protection, PIN-authenticated signed URL generation (private bucket, 60s minimum TTL, 300s default), server-side UUID path randomization with client-reconciled upload path persistence, resilient multi-directory fallback resolution with automated database self-healing (`get_document_url`), and inline in-browser secure tab previewer (rendering images with download controls and PDFs in full-viewport iframes, preventing Chromium top-frame blob URL navigation restrictions). Clickable document attachment badges (`📎 {count} Doc(s)`) with tactile loading indicators and error toasts across all holding cards (Insurance, Gold, Real Estate, FD, RD, SIP, Document Vault).
...
* **Verification Pipeline**:
  ...
  * `npm test` (`vitest run`) executes the complete test suite across 55 test files and 309 unit/integration test cases (100% passing).
...
4. **Zero-Latency Touch & GPU Layer Promotion**: Global `touch-action: manipulation` eliminates the 300ms mobile tap delay. Fixed bars (`.mobile-status-bar`) and the Liquid Glass tab bar promote to GPU compositor layers (`transform: translateZ(0); will-change: transform`).
...
7. **Screenshot Refresh & Purge Protocol**: Whenever asked to capture or update any screenshot (single view or complete project suite): ALWAYS remove the old version(s) first from the target directories (`screenshots/web/`, `screenshots/mobile/`). Maintain identical dimensions and 2x Retina accuracy (Desktop: 1920×1080 @ 2x, Mobile: 390×844 @ 2x, fullPage: true) across both Light and Dark modes.
...
| :--- | :--- | :--- |
| **2026-09-15** | `v3.0` | **WhatsApp iOS 27 Liquid Glass Mobile Navigation Redesign**: (1) Redesigned `MobileBottomNav.tsx` from solid-backdrop to full iOS 27 Liquid Glass — tab bar uses `backdrop-filter: blur(28px) saturate(1.8)` at 76% surface opacity with an ultra-thin 0.5px border; (2) Replaced flat active-tab highlight with a springy pill indicator using `cubic-bezier(0.34, 1.56, 0.64, 1)` spring physics (`scale 0.5→1`) and `1.1×` icon scale-up on active; (3) Replaced `Menu` hamburger with `LayoutGrid` icon that rotates `45°` on open; (4) More drawer upgraded to a two-phase animated frosted bottom sheet (`blur(34px) saturate(1.8)`, iOS-native `cubic-bezier(0.32, 0.72, 0, 1)` spring) with frosted `rgba(0,0,0,0.44)` + `blur(10px)` backdrop; (5) Each More-drawer category now has a unique accent color (cyan, amber, green, purple, orange, red) with colored icon squares that fill solid with a glow shadow when active. |
