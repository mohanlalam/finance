...
* **Portfolio Table Enhancements**:
  * **[PortfolioTable.tsx](file:///c:/Users/Ram%20Mohan/OneDrive/Desktop/project%20antigravity/src/components/PortfolioTable.tsx)**: Features quick filter pills (*All*, *Gainers*, *Losers*, *ETFs*) with item count badges and interactive column sorting (Ticker, Value, Today's P&L, Total P&L) with directional arrow indicators (▲/▼). Compacted table padding (`px-2`) and stock name truncation (`max-w-[140px]`) fit all 11 columns in full-width desktop view without horizontal scrollbars. Both desktop table and mobile card views respect the active filter and sort state.
* **Export & PDF**:
